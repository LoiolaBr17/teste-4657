...
async function createCart () {
  try {
    cartId = -1;
    const add = await axios.post(`https://babyemommy.com.br/api/cartService/getCart/${cartId}/${idClient}`, {
  ...
  try {
    if (idClient) {
      if (cartId === -1) {
        ...
        const add = await axios.post(`https://babyemommy.com.br/api/cartService/addCartItem/${cartId}`, {
          quantity: qtdProduct.value,
          productId: produto.value.id
        });
        console.log(add);
