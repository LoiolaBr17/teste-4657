...
function checaLogin () {
  if (loginSalvo && senhaSalva) {
    window.location.href = `https://babyemommy.com.br/elevarone.com.br/checkout?idCart=${cartId}`;
  } else {
    $q.notify({
      color: "red-5",
      textColor: "white",
...
async function clearCart () {
  try {
    const response = await axios.post(`https://babyemommy.com.br/api/cartService/clearCart/${cartId}`);
